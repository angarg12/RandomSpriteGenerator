...
class ImageUtils {
	...
	private static void writePNGChunk(OutputStream out, byte[] data, int len)
			throws IOException {
		byte[] buf = new byte[4];
	...
	/**
	 * Update a running CRC with the bytes buf[ofs..ofs+len-1]. The CRC should
	...
	public static void writePAM(String filename, int[][] pix, int width,
			int height) {
		try {
			// Create file
			FileOutputStream out = new FileOutputStream(filename);
