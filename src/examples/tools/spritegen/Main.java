...
public class Main extends JGEngine {
	...
	public void initGame() {
		...
		/*defineImageFromData("test","T",0,
			4,4, new int[] {
				0xffffffff,0xfff00fff,0x00000000,0xffffffff,
				0xffffffff,0xffffffff,0x00000000,0xffffffff,
				0xffffffff,0x00ffffff,0x00000000,0xffffffff,
				0xffffffff,0x00ffffff,0x00000000,0xffffffff,
			}, 0, "", -1,-1,-1,-1);*/
	...
	public void paintFrame() {
		...
		if (seltimer>0) {
			setColor(new JGColor(seltimer/15.0,seltimer/15.0,seltimer/15.0));
		...
		if (imgdef && getMouseButton(1)) {
			...
			&&  getMouseY()>=10 && getMouseY() < 10+NRSPRY*SPRY) {
				sprnr = ( (getMouseX()-10) / SPRX )
					+ NRSPRX*( (getMouseY()-10) / SPRY );
				if (sprnr >=0 && sprnr < NRSPR) {
					...
					if (isAndroid()) {
						...
						try {
							...
							File myjar = new File(URLDecoder.decode(path, "UTF-8"));
							mypath = myjar.getParentFile().getPath();
			                //mypath = Main.class.getProtectionDomain().getCodeSource().getLocation().toURI().getPath();
						} catch (Exception e) {
							e.printStackTrace();
						}
					}
					String filename = "spr"+rest+"-"
						+formatString(2,min)+"-"
						+formatString(2,sec)+"."
						+formatString(3,msec);
					ImageTools.writePNG(mypath+File.separator+filename+".png",
						selspr.pixels,selspr.pixels.length,
		...
		if (!imgdef || regen) {
			...
			if (selspr!=null) {
				...
				shape.shading = random(0,1) < 0.75 
						? 0 : random(0,1) < 0.5 ? 1 : 2;
			...
			for (int i=0; i<NRSPR; i++) {
				...
				*/
				...
				sprites[i] = shape.mergeSprites(sprt, shape.createSprite(coltable), 0.9);
				if (selspr!=null) {
					if (sprnr==i) {
						sprites[i] = selspr;
					} else {
						sprites[i] = shape.mergeSprites(selspr,sprites[i],0.7);
					}
				}
				int startframe=0;
				int nrframes=1;
				...
				for (int f=startframe; f<sprites[i].getNrFrames(); f++) {
					...
					/*defineImageFromData(frames[f-startframe],"-",0,
						2*sprites[i].getWidth()/sprites[i].getNrFrames(),
						2*sprites[i].getHeight(),
						sprites[i].getHq2xData(f),
						0,2*sprites[i].getWidth()/sprites[i].getNrFrames(),
						"", -1,-1,-1,1);*/
					defineImageFromData(frames[f-startframe],"-",0,
						sprites[i].getWidth()/sprites[i].getNrFrames(),
						sprites[i].getHeight(),
						sprites[i].getData(f),
						0,sprites[i].getWidth()/sprites[i].getNrFrames(),
						"", -1,-1,-1,1);
				}
				// XXX animations not undefined: memory leak
				defineAnimation("rand"+i, frames, random(0.2,0.5));
				new JGObject("rand",true,10+x*SPRX,10+y*SPRY,0,"rand"+i);
				x ++;
				if (x>=NRSPRX) {
					x=0;
					y++;
		...
		/*for (int i=0; i<NRSPR; i++) {
			drawImage("rand"+i,10+x*SPRX,30+y*SPRY);
			x ++;
			if (x>=NRSPRX) {
				x=0;
				y++;
